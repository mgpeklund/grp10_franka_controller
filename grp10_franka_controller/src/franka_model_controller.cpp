...
#include <grp10_franka_controller/franka_model_controller.h>
...
namespace grp10_franka_controller {
	...
	bool FrankaModelController::init(hardware_interface::RobotHW* robot_hw,
			ros::NodeHandle& node_handle) {

		
		franka_state_interface_ = robot_hw->get<franka_hw::FrankaStateInterface>();
		if (franka_state_interface_ == nullptr) {
			ROS_ERROR("FrankaModelController: Could not get Franka state interface from hardware");
			return false;
		}
		std::string arm_id;
		if (!node_handle.getParam("arm_id", arm_id)) {
			ROS_ERROR("FrankaModelController: Could not read parameter arm_id");
			return false;
		}
		model_interface_ = robot_hw->get<franka_hw::FrankaModelInterface>();
		if (model_interface_ == nullptr) {
			ROS_ERROR_STREAM("FrankaModelController: Error getting model interface from hardware");
			return false;
		}
		...
		try {
			...
			ROS_ERROR_STREAM(
					"FrankaModelController: Exception getting franka state handle: " << ex.what());
			return false;
		}
		...
		try {
			...
			ROS_ERROR_STREAM(
					"FrankaModelController: Exception getting model handle from interface: " << ex.what());
			return false;
		}
		return true;
	}

	void FrankaModelController::update(const ros::Time& /*time*/, const ros::Duration& /*period*/) {
...
PLUGINLIB_EXPORT_CLASS(grp10_franka_controller::FrankaModelController,
